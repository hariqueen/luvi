 ...
 * 고객에게 — 운영자가 답변했을 때.
 *
 * 🔴 **답변 전문을 본문에 넣습니다.** 링크만 보내면 답을 읽는 데 클릭이 한 번 더 듭니다.
 *    문의한 사람은 이미 한 번 막혀서 온 사람입니다.
 *
 * 🔴 **광고를 한 줄도 섞지 마세요.** 이 메일은 거래·서비스 메일이라 수신동의 없이 보낼 수
 *    있지만, 프로모션이 한 줄이라도 들어가면 광고성 정보가 되어 사전 동의와 야간 발송 제한
 *    규제를 받습니다. 서명 줄에 신상품 안내를 붙이고 싶어지면 멈추세요.
 *
 * ⚠️ **조회 링크를 다시 만들어 넣을 수 없습니다.** 서버는 토큰의 **해시만** 보관하므로
 *    원문을 복원할 방법이 없습니다(그게 의도입니다). 새로 발급하면 고객이 이미 받아둔
 *    접수 메일의 링크가 죽습니다. 그래서 접수 메일의 링크를 쓰라고 안내만 합니다.
 */
export function replyMail(
  env: MailEnv,
  input: { number: string; name: string; to: string; body: string },
): SendInput {
  return {
    to: input.to,
    subject: `문의하신 내용에 답변드렸습니다 (${input.number})`,
    replyTo: SUPPORT_ADDRESS,
    text: `${input.name}님, 기다려 주셔서 감사합니다.
...
이 메일에 그대로 답장하시면 같은 담당자에게 전달됩니다.
접수 확인 메일에 있던 조회 링크에서도 전체 대화를 보실 수 있습니다.
